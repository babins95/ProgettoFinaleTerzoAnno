...
[RequireComponent(typeof(LaserBeam), typeof(PolygonCollider2D))]
public class LaserCollision : MonoBehaviour
{
    ...
    {
        if (positions != laserBeam.GetPositions())
        {
            //Get all the positions from the line renderer
            positions = laserBeam.GetPositions();
    ...
    private List<Vector2> CalculateColliderPoints(List<Vector2> positions)
    {
        ...
        float width = laserBeam.GetWidth();
